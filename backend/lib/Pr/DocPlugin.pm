...
sub register {
  ...
  my $preprocess = $conf->{preprocess} || 'bwtr';
  ...
  $app->routes->any(
      ...
      $module =~ s/\//\:\:/g;      
      if ($module eq 'Pr::Cfg'){
          $html = _pod_to_html(Pr::Config->pod);
      }
      else {
          my $path;
          if ($module eq 'Pr::LocalGuide'){
...
Pr::DocPlugin - Extopus Documentation Plugin
